...
<card v-bind="card" /> * * This "spreads" all the attributes of `card`
individually as props to <card />. * Note also that @click fires 2 methods, one
to call toggle() and the other to: * * $emit('set-name', card.name) * * This
provides data back up to the parent app. */
<template>
  <div class="row">
    <div
      v-for="card in cards"
